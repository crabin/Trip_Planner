...
- 更新根目录 `README.md` 的项目展示部分，统一引用 `assets/showcase/` 中的规划页、行程结果页、深度规划研究过程、历史行程和浮动聊天助手素材。
- 重写 `assets/showcase/README.md` 为展示素材索引，补充截图、视频和深度规划报告样例的用途说明与预览。
- 在 `frontend/README.md`、`backend/README.md` 和两个 Agent README 中补充项目展示入口，保持各模块文档都能跳转到同一组展示素材。
- 校验 README 中的本地图片、视频和 Markdown 样例链接，确保中文文件名与带空格文件名的相对路径可解析。

## 2026-06-15

### 地图服务增强

- 增强高德 POI 响应解析，兼容 v3 与 v5 的 POI 列表结构。
- 新增 POI 标准化字段：评分、参考消费、标签、电话、距离、图片、经纬度与地址。
- 新增周边搜索能力，支持基于景点坐标推荐附近住宿和餐饮。
- 住宿推荐优先使用当天第一个已定位景点作为参考点，提升酒店位置与行程的贴合度。
- 餐饮推荐会识别"特色餐饮、午餐推荐、晚餐推荐"等泛化名称，并用附近真实餐厅 POI 替换。
- 地图 enrich 完成后会在来源说明中记录已补充地址、坐标、评分、参考消费、附近餐饮住宿或路线估算信息。

### 行程生成与预算优化

- 从 RAG 攻略片段中解析景点门票信息，生成景点预算时优先使用本地攻略价格。
- 景点描述中自动补充"门票参考本地攻略"说明，增强预算来源可解释性。
- 新增跨目的地提示过滤，避免成都、大理、西安、厦门、三亚之间的典型景点词互相污染。
- 保留原有规则估价作为兜底，当攻略片段缺少门票价格时继续使用启发式估算。

### 数据模型与测试

- 后端 `SpotItem`、`MealItem`、`HotelItem` 扩展地图字段，前端 TypeScript 类型同步更新。
- 新增地图服务测试，覆盖 POI 解析、周边餐饮住宿推荐、v5/v3 降级和缓存等关键路径。
- 扩展行程服务测试，覆盖门票参考解析、跨目的地提示清理和生成结果字段一致性。
- 更新 RAG 检索评估脚本与测试，保持目的地过滤、rerank 和检索结果统计可验证。
