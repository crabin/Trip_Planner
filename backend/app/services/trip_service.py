...
def _build_demo_spot_names(destination: str, rag_contexts: list[str], day_count: int) -> list[str]:
    ...
    destination_spot_names = {
        "大理": ["大理古城", "喜洲古镇", "崇圣寺三塔", "洱海生态廊道"],
        "成都": ["大熊猫繁育研究基地", "宽窄巷子", "都江堰", "青城山"],
        "西安": ["秦始皇兵马俑博物馆", "西安城墙", "大雁塔", "陕西历史博物馆", "回民街"],
        "厦门": ["鼓浪屿", "南普陀寺", "厦门大学", "环岛路", "曾厝垵"],
        "三亚": ["亚龙湾", "蜈支洲岛", "天涯海角", "南山文化旅游区", "亚特兰蒂斯水世界"],
    }
    for spot_name in destination_spot_names.get(destination, []):
        if spot_name in joined_context:
            candidate_names.append(spot_name)
...
def _build_contextual_cycling_tip(destination: str, rag_contexts: list[str]) -> str | None:
    """根据当前目的地的攻略片段生成骑行提示，避免把洱海写进其他城市。"""
    joined_context = "\n".join(rag_contexts)
    cycling_spots_by_destination = {
        "大理": "洱海生态廊道",
        "厦门": "环岛路",
    }
    spot_name = cycling_spots_by_destination.get(destination)
    if spot_name and "骑行" in joined_context and spot_name in joined_context:
        return f"本地攻略提到{spot_name}适合骑行，可作为第二天或第三天备选。"
    return None
...
def generate_trip_itinerary(request: TripRequest) -> Itinerary:
    ...
    cycling_tip = _build_contextual_cycling_tip(request.destination, rag_contexts)
    if cycling_tip:
        tips.append(cycling_tip)
