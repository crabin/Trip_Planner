"""Translate trip-planning intent into local-guide retrieval queries."""
...
def _extract_note_keywords(
    ...
    for triggers, required_destination, values in rules:
        if required_destination and destination and required_destination not in destination:
            continue
...
def _build_destination_query(
    ...
    return build_destination_query(destination, preferences, pace, special_notes)
...
def get_destination_guide_context(
    ...
    return retrieve_travel_guide(query=query, top_k=top_k, destination=destination)
