...
最终输出边界：
- 只输出一份完整、独立、可直接分享的 Markdown 攻略，不得省略上述任何一级章节。
- 不得包含追问、邀请用户回复、征求是否继续、承诺下一步服务或“如果你愿意/如果你需要/我可以继续/下一步可以”等对话式话术。
- 不要把未完成的交通、酒店、餐厅候选推迟到下一轮对话；资料不足时必须在对应章节给出候选筛选结构、已知选项和明确待确认项。
- 文档必须结束于“资料来源与更新说明”中的临行复核提醒，不要在其后追加任何内容。
"""
