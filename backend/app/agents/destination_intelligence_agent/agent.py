...
class DestinationIntelligenceAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        """
        初始化Destination Intelligence Agent
    ...
    def _initialize_llm(self) -> LLMClient:
        """初始化LLM客户端"""
        return LLMClient(
            api_key=self.config.DESTINATION_INTELLIGENCE_AGENT_API_KEY,
            model_name=self.config.DESTINATION_INTELLIGENCE_AGENT_MODEL_NAME,
            base_url=self.config.DESTINATION_INTELLIGENCE_AGENT_BASE_URL,
        )
...
def create_agent() -> DestinationIntelligenceAgent:
    """
    创建Destination Intelligence Agent实例的便捷函数
    
    Returns:
        DestinationIntelligenceAgent实例
    """
    from .utils.config import Settings
    config = Settings()
    return DestinationIntelligenceAgent(config)
